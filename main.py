...
class MEAIE:
    ...
    def __init__(self):
        ...
        self.attr_max_num = None
        self.eid_mask_list = None
        self.eav_len_list = None
        self.eid_vid_list = None
        self.eid_aid_list = None
        self.eid_aid_v_add2 = None
        self.eid_aid_v_add1 = None
        self.eid_aid_v2 = None
        self.eid_aid_v1 = None
        self.attr_id_dict2 = None
        self.attr_id_dict1 = None
        self.attr_list = None
    ...
    def parse_options(parser):
        ...
        # multi loss params    6改成了7
        parser.add_argument("--inner_view_num", type=int, default=7, help="the number of inner view")
    ...
    def init_data(self):
        ...
        self.attr_list, self.attr_id_dict1, self.attr_id_dict2, \
            kg1_attribute_triples_set, kg2_attribute_triples_set = generate_attr_id('data/FB15K_DB15K/numerical_tris1', 'data/FB15K_DB15K/numerical_tris2')
        _, self.eid_aid_v1 = uris_attribute_triple_2ids(kg1_attribute_triples_set, self.ent2id_dict,
                                                             self.attr_id_dict1)
        _, self.eid_aid_v2 = uris_attribute_triple_2ids(kg2_attribute_triples_set, self.ent2id_dict,
                                                             self.attr_id_dict2)
        self.eid_aid_v_add1, self.eid_aid_v_add2 = add_sup_attribute_triples(self.train_ill, self.eid_aid_v1,
                                                                                  self.eid_aid_v2)
        self.attr_max_num, self.eid_aid_list, self.eid_vid_list, self.eav_len_list, self.eid_mask_list = self.generate_attr_list(
            self.eid_aid_v_add1, self.eid_aid_v_add2)
    ...
    def init_model(self):
        ...
        self.multimodal_encoder = MultiModalEncoder(args=self.args,
                                                    ...
                                                    attr_list=self.attr_list).to(self.device)
    ...
    def semi_supervised_learning(self):
        ...
        with torch.no_grad():
            ...
            name_emb, char_emb,att_num_emd, joint_emb = self.multimodal_encoder(self.input_idx,
                                                                    self.adj,
                                                                    self.img_features,
                                                                    self.rel_features,
                                                                    self.att_features,
    ...
    def inner_view_loss(self, gph_emb, rel_emb, att_emb, img_emb, name_emb, char_emb,att_num_emd, train_ill):
        ...
        loss_num = self.criterion_cl(att_num_emd, train_ill) if att_num_emd is not None else 0

        total_loss = self.multi_loss_layer([loss_GCN, loss_rel, loss_att, loss_img, loss_name, loss_char, loss_num])
        return total_loss

    def kl_alignment_loss(self, joint_emb, gph_emb, rel_emb, att_emb, img_emb, name_emb, char_emb,att_num_emd, train_ill):
        ...
        loss_num = self.criterion_align(att_num_emd, joint_emb, train_ill) if att_num_emd is not None else 0

        total_loss = self.align_multi_loss_layer(
                [loss_GCN, loss_rel, loss_att, loss_img, loss_name, loss_char, loss_num]) * zoom
        return total_loss
    ...
    def train(self):
        ...
        # add new att_num !!!
        # print('self.train_ill111', [self.eid_aid_list[x] for x in self.train_ill])
        # print('self.train_ill112222221', self.eid_aid_list[0], self.eid_aid_list[1])
        self.entity_num_attr1 = torch.LongTensor([self.eid_aid_list[x] for [x, _] in self.train_ill]).cuda()
        self.entity_num_val1 = torch.FloatTensor([self.eid_vid_list[x] for [x, _] in self.train_ill]).cuda()
        self.mask_num1 = torch.ByteTensor([self.eid_mask_list[x] for [x, _] in self.train_ill]).cuda()
        self.l1_num1 = torch.FloatTensor([self.eav_len_list[x] for [x, _] in self.train_ill]).cuda()
        self.entity_num_attr2 = torch.LongTensor([self.eid_aid_list[x] for [_, x] in self.train_ill]).cuda()
        self.entity_num_val2 = torch.FloatTensor([self.eid_vid_list[x] for [_, x] in self.train_ill]).cuda()
        self.mask_num2 = torch.ByteTensor([self.eid_mask_list[x] for [_, x] in self.train_ill]).cuda()
        self.l1_num2 = torch.FloatTensor([self.eav_len_list[x] for [_, x] in self.train_ill]).cuda()
        self.entity_num_attr = torch.cat((self.entity_num_attr1, self.entity_num_attr2))
        self.entity_num_val = torch.cat((self.entity_num_val1, self.entity_num_val2))
        self.mask_num = torch.cat((self.mask_num1, self.mask_num2))
        self.l1_num = torch.cat((self.l1_num1, self.l1_num2))
        ...
        for epoch in range(self.args.epochs):
            ...
            for si in np.arange(0, self.train_ill.shape[0], bsize):
                ...
                in_loss = self.inner_view_loss(gph_emb, rel_emb, att_emb, img_emb, name_emb, char_emb,att_num_emd,
                ...
                align_loss = self.kl_alignment_loss(joint_emb, gph_emb, rel_emb, att_emb, img_emb, name_emb,
                                                    char_emb, att_num_emd, self.train_ill[si:si + bsize])
            ...
            del joint_emb, gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb,att_num_emd
    ...
    def test(self, epoch):
        with torch.no_grad():
            ...
            if "100" in self.args.file_dir:
                ...
                del distance, gph_emb, img_emb, rel_emb, att_emb, name_emb, char_emb,att_num_emd, joint_emb
                gc.collect()
